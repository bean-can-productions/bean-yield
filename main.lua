...
Logo.Image = "rbxassetid://13246625548"
...
CMDs[#CMDs + 1] = {NAME = 'bypass', DESC = 'Bypasses ExaCheat'}
...
CMDs[#CMDs + 1] = {NAME = 'setgatetext [text]', DESC = 'Sets certain boards in Scottish Connnect.'}
CMDs[#CMDs + 1] = {NAME = 'gearall [id]', DESC = 'Gives everyone gear at SC.'}
CMDs[#CMDs + 1] = {NAME = 'gearself [id]', DESC = 'Gives yourself gear at SC.'}
CMDs[#CMDs + 1] = {NAME = 'scaudio [id]', DESC = 'Plays audio at SC.'}
CMDs[#CMDs + 1] = {NAME = 'nuke', DESC = 'Nukes the fucking game (SC)'}
wait()
...
addcmd('setgatetext',{},function(args, speaker)
...
workspace:WaitForChild("Advanced Gate Package (DONT UNGROUP)"):WaitForChild("Events"):WaitForChild("InfoHandler"):FireServer(unpack(args))
...
addcmd('gearall',{},function(args, speaker)
   ...
   if seEvent then
			seEvent:FireServer({ name = "Gear", id = gatetext, toAll = true })
		end
end)
...
addcmd('gearself',{},function(args, speaker)
   ...
   if seEvent then
			seEvent:FireServer({ name = "Gear", id = gatetext, toAll = false })
		end
end)
...
addcmd('scaudio',{},function(args, speaker)
  ...
  if seEvent then
			seEvent:FireServer({ name = "Audio", id = gatetext })
		end
end)
...
if ReplicatedStorage:FindFirstChild("Remotes") and ReplicatedStorage:FindFirstChild("exaModule") then
    ReplicatedStorage:WaitForChild("Remotes").Name = "Troll"
    notify('Bypassed ExaCheat succesfully.')
else
    notify('Remotes Folder not found. Game is not by AQ or bypass has been patched.')
end
end)
